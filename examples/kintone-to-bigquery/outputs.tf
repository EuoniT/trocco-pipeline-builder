output "job_definition_id" {
  ...
  value       = trocco_job_definition.kintone_to_bigquery.id
}
...
output "job_name" {
  ...
  value       = trocco_job_definition.kintone_to_bigquery.name
}
...
output "pipeline_summary" {
  ...
  value = {
    ...
    destination = "bigquery (${var.bq_dataset}.${var.bq_table})"
